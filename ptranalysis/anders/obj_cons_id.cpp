...
#if DEBUG_OCI == 0
  #undef DEBUG_TYPE
  #define DEBUG_TYPE "___disabled"
#endif
#if FIELD_SENSITIVE != 1
  #warning TODO: implement field-insensitive mode
#endif
...
void Anders::obj_cons_id(){
  ...
  /*for(StringMap::iterator it= map.begin(), ie= map.end(); it != ie; ++it)
    if(const StructType* ST= dyn_cast<StructType>(it->second))
      _analyze_struct(ST);
*/
        
  StringMap<StructType *>& map = curr_module->getContext().pImpl->NamedStructTypes;
  for(StringMap<StructType *>::iterator it = map.begin(); it!=map.end(); it++){
      StructType* struct_type = it->second;
      _analyze_struct(struct_type);
      
       // const FindUsedTypes& used_types = getAnalysis<FindUsedTypes>();
       /*
        const SetVector<Type*> types = used_types.getTypes();
        for (SetVector<Type*>::const_iterator it = types.begin();
                it != types.end(); it++) {
            Type* type = *it;
            if (isa<StructType>(type)) {
                type->_analyze_struct();
            }
        }
        */
  }
  ...
  for(Module::global_iterator it= curr_module->global_begin(),
    ...
    }else if(CHECK_GLOBAL_NULL){
      if(!ng_hdr){
        ng_hdr= 1;
        PUTS("!! uninitialized globals:\n");
      }
      print_val_now(G);
      putc('\n', stderr);
    }
  ...
  verify_nodes();
  //The original nodes and constraints should appear
...
void Anders::check_cons_undef() const{
  ...
  for(DenseSet<Value*>::const_iterator it= at_args.begin(), ie= at_args.end();
      it != ie; ++it){
  ...
  FORN(i, constraints.size()){
    ...
    if(S){
      if(isa<Argument>(S)){
        ...
        continue;//PUTS("[arg] ");
      }else if(!strcmp(S->getName().str().c_str(), "UnifiedRetVal")){
...
u32 Anders::get_val_node(Value *V, bool allow_null) const{
  assert(V);
...
u32 Anders::get_obj_node(Value *V, bool allow_null) const{
  assert(V);
...
u32 Anders::get_ret_node(Function *F) const{
  assert(F);
...
u32 Anders::get_vararg_node(Function *F) const{
  assert(F);
...
void Anders::id_alloc_insn(Instruction *I){
  ...
  if(const StructType *ST= dyn_cast<StructType>(T)){
    ...
    //FIXME: any field of array type should also be weak.
    FORN(i, nf){
...
void Anders::id_ind_call(CallSite CS){
  ...
  for(; itA != ieA; ++itA, ++arg_off){
    ...
    DPUTS("  ");
    DEBUG(print_val_now(AA));
    //NOTE: if the current formal arg in the function ptr type is of non-ptr
    //  type, we still add the store_cons, but the solver will ignore it.
    if(isa<PointerType>(AA->getType())){
      u32 vnAA= get_val_node_cptr(AA);
      if(vnAA){
  ...
  //NOTE: indirect calls of varargs are not supported because this is a very
  //  uncommon case. To handle this, whenever the offset on a store cons. is
  //  exceeded, the solver will need to reset it to the vararg node offset if
  //  the current member of the destination points_to is a vararg func.
  DEOL;
}
  ...
  switch(tF){
    ...
    case EFT_A11R_NEW:{
      ...
      if(const StructType *ST= dyn_cast<StructType>(T)){
        ...
        u32 nf= sz.size();
        FORN(i, nf){
          //NOTE: X/0 shouldn't really have a value because it's not
          //  pointed to by any program variable, but for now we require
          //  all obj_nodes to have one.
          //The node is weak because it's heap-allocated.
