...
const char* BackSlice::initFileName = "slice.conf";

static RegisterPass<BackSlice> A("back-slice", "Backward Slice Pass",
...
void BackSlice::doRangePruning(Instruction i)
{


}

bool BackSlice::runOnModule(Module &M)
{
    ...
    readInit(sdg, M, fin);
    fin.close();
    markVerticesOfBackSlice(sdg, markedNodes);
    // TODO: implement
    sliceModule(sdg, M);
    // errs() << M << "\n";
    return true;
...
void BackSlice::sliceModule(SDG &sdg, Module &M)
{
...
bool BackSlice::markVerticesOfBackSlice(SDG &sdg, BackSlice::nodeSet_t &resultSet)
{
    ...
    markReachingVerticesForBackSlice(sdg, resultSet, nodeSet,
            control | flow | call | paramIn);
    // Step 2: Slice called procedures, without aschending to call sites
    markReachingVerticesForBackSlice(sdg, resultSet, resultSet,
...
bool BackSlice::markReachingVerticesForBackSlice(SDG &sdg, BackSlice::nodeSet_t &resultSet,
        BackSlice::nodeSet_t workSet, int mask)
{
    while (!workSet.empty())
    {
...
void BackSlice::readInit(SDG &sdg, Module &M, std::istream &in)
{
